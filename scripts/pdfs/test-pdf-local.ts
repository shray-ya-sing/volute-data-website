...
// ---------------------------------------------------------------------------
// Send to LOCAL endpoint and save PDF
// ---------------------------------------------------------------------------
...
const req = http.request(options, (res) => {
  ...
  res.on('end', () => {
    ...
    if (res.statusCode === 200 && contentType.includes('application/pdf')) {
      const outputPath = path.resolve('./slide-output-local.pdf');
      fs.writeFileSync(outputPath, body);
      console.log(`[test] ✅ PDF saved to: ${outputPath}`);
      console.log(`[test] PDF size: ${(body.length / 1024).toFixed(1)} KB`);
    } else if (res.statusCode === 200 && contentType.includes('image/png')) {
      const outputPath = path.resolve('./slide-output-local.png');
      fs.writeFileSync(outputPath, body);
      console.log(`[test] ✅ PNG saved to: ${outputPath}`);
      console.log(`[test] PNG size: ${(body.length / 1024).toFixed(1)} KB`);
    } else {
      ...
      try {
        ...
        console.error('[test] ❌ Raw response:', body.toString().slice(0, 2000));
      }
      process.exit(1);
    }
  });
...
req.on('error', (err) => {
  ...
  if ((err as any).code === 'ECONNREFUSED') {
    console.error('[test] Is your local server running? Start it with: npx vercel dev --listen 3001');
  }
  process.exit(1);
});
