...
function readVendor(filename: string): string {
  ...
  if (!fs.existsSync(filePath)) {
    throw new Error(
      `[export-png] Missing vendor file: ${filePath}\n` +
...
console.log('[export-png] Vendor bundles loaded:', {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  console.log(`[export-png] Rendering ${slideArray.length} slide(s) as PNG...`);
  ...
  try {
    ...
    if (isVercel) {
      try {
        ...
        console.log(`[export-png] Using @sparticuz/chromium at: ${executablePath}`);
      } catch (err) {
        throw new Error(`@sparticuz/chromium is required on Vercel but failed to load: ${err}`);
      }
    } else {
      console.log('[export-png] Local dev mode, using playwright installed browser');
    ...
    for (let i = 0; i < slideArray.length; i++) {
      ...
      console.log(`[export-png] Rendering slide ${slideLabel}...`);
      ...
      console.log(`[export-png] Slide ${slideLabel} captured (${(screenshotBuffer.length / 1024).toFixed(1)} KB)`);
    }

    await browser.close();
    browser = null;

    // ── Single slide → return the PNG directly (no zip overhead) ──────────
    if (slideBuffers.length === 1) {
      const { buffer, slideNumber } = slideBuffers[0];
      res.setHeader('Content-Type', 'image/png');
      res.setHeader('Content-Disposition', `attachment; filename="${filename}-slide-${slideNumber}.png"`);
    ...
    for (const { buffer, slideNumber } of slideBuffers) {
      ...
      zip.file(`${filename}-slide-${paddedNum}.png`, buffer);
    }
    ...
    console.log(`[export-png] ZIP created: ${slideBuffers.length} slides, ${(zipBuffer.length / 1024).toFixed(1)} KB`);
    ...
    console.error('[export-png] Error:', error.message);
    console.error('[export-png] Stack:', error.stack);
    if (browser) {
